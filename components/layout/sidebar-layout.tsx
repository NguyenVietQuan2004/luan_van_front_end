...
export default function SidebarLayout({ children, navItems }: { children: ReactNode; navItems: NavItem[] }) {
  ...
  // 👇 tự động đóng nếu route = /abc
  useEffect(() => {
    if (pathname.startsWith("/trich-xuat")) {
      setOpen(false);
    } else {
      setOpen(true);
    }
        ...
        <div className="text-[26px] font-semibold">
          <Link href={"/"} className="flex items-center">
            <img
              ...
              className="w-12 h-12 mx-4"
            />
            <span>Chi bộ Khoa học máy tính </span>
          </Link>
        </div>
      </header>
          ...
          <nav className="p-4 space-y-2 text-sm">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`transition-all duration-300 block px-4 py-2 rounded-lg
                ${pathname === item.href ? "bg-[#e6f4ff] text-[#1e5aaa]" : "hover:bg-gray-200"}`}
              >
                {item.label}
              </Link>
            ))}
