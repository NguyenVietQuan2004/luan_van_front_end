...
// Hàm kiểm tra đảng viên có đang được miễn đảng phí không (tháng hiện tại)
const isMienDangPhi = (row: any): boolean => {
  const mienList = row.dangvien_phi_id?.mien_dang_phi || [];
  if (mienList.length === 0) return false;

  // === SỬA Ở ĐÂY: Dùng tháng/năm từ bản ghi thay vì new Date() ===
  ...
  return mienList.some((m: any) => {
    ...
    // Kiểm tra khoảng thời gian miễn có bao phủ tháng/năm của bản ghi không
...
export const DangPhiColumns: ColumnDef<DangPhi>[] = [
  ...
  // === CỘT MỚI: ĐƯỢC MIỄN THÁNG HIỆN TẠI ===
  {
    id: "duoc_mien",
