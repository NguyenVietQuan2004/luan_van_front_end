...
//   // Dữ liệu chỉnh sửa tạm thời (chỉ lưu khi bấm nút Lưu)
...
//       // Load dữ liệu nộp cấp trên theo năm
//       const resNop = await fetch(`${API_NOP}?nam=${selectedNam}`);
//       if (resNop.ok) {
//         const dataNop: NopDangPhiCapTren[] = await resNop.json();
//         setNopCapTrenList(dataNop);

//         // Khởi tạo editData từ dữ liệu backend
...
//   // === Hàm kiểm tra đảng viên có đang được miễn không ===
...
//   // Tính toán tổng hợp cho Phần II + merge dữ liệu từ editData
...
//           thang,
//           nam: selectedNam,
//           ngay_nop: data.ngay_nop || new Date().toISOString().split("T")[0], // required
//           trich_giu_lai: data.trich_giu_lai || Math.round(tongHopTheoThang[thang - 1].daThu * 0.4),
//           phai_nop_cap_tren: data.phai_nop_cap_tren || Math.round(tongHopTheoThang[thang - 1].daThu * 0.6),
...
//       await loadData(); // Reload để đồng bộ dữ liệu
//     } catch (err: any) {
//       console.error(err);
...
//         "Thu nhập tính đóng đảng phí": mien ? 0 : dp.thu_nhap || 0,
//         "Đảng phí tháng này": mien ? 0 : dp.dang_phi || 0,
//         "Truy thu các tháng trước": mien ? 0 : dp.truy_thu || 0,
//         "Cộng đảng phí": mien ? 0 : dp.tong_dang_phi || 0,
//         "Đã thu": mien ? 0 : dp.da_thu || 0,
//         "Chưa đóng": mien ? 0 : dp.tong_dang_phi - (dp.da_thu || 0) || 0,
//       };
//     });
...
//     // PHẦN II (dùng dữ liệu đã chỉnh sửa)
//     const phanII = tongHopTheoThang.map((m) => ({
//       Tháng: m.thang,
//       "Ngày tháng năm nộp": m.displayNgay,
...
//       {/* PHẦN II - ĐÃ ĐƯỢC CẬP NHẬT ĐỂ NHẬP DỮ LIỆU */}
//       {selectedThang == 0 && (
//         <div>
...
//             disabled={saving}
//             className="bg-[#3872B2] my-2 text-white px-4 py-1.5 rounded text-[13px]  disabled:opacity-70"
//           >
...
//                     {/* Cột Đã nộp cấp trên - có thể chỉnh */}
//                     <td className={td}>
//                       <input
//                         type="number"
//                         value={m.daNop}
...
export default function SoThuNopDangPhiPage() {
  ...
  // Chỉ lưu dữ liệu chỉnh sửa tạm thời của năm hiện tại
  const [editData, setEditData] = useState<Record<number, Partial<NopDangPhiCapTren>>>({});
  ...
  // Chỉ load lại Phần I khi đổi tháng (không reset editData)
  useEffect(() => {
    loadData();
  }, [selectedThang]);
