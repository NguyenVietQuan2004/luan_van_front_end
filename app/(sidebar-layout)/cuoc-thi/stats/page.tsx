...
export default function ContestStatsPage() {
  ...
  const [selectedMonth, setSelectedMonth] = useState<number | "all">("all");
  const [selectedYear, setSelectedYear] = useState<number | "all">("all");
  const [selectedContest, setSelectedContest] = useState<string>(""); // Filter theo tên cuộc thi
  ...
  const filteredStats = useMemo(() => {
    return stats.filter((item) => {
      ...
      const matchYear = selectedYear === "all" || item.year === selectedYear;
      const matchContest =
        selectedContest.trim() === "" || item.contest_name.toLowerCase().includes(selectedContest.toLowerCase().trim());

      return matchMonth && matchYear && matchContest;
    });
  }, [stats, selectedMonth, selectedYear, selectedContest]);
  ...
  const exportToExcel = () => {
    ...
    grouped.forEach((group: any) => {
      group.items.forEach((row: ContestStat) => {
        excelData.push({
          "Đảng viên": group.ho_ten,
          "Cuộc thi": row.contest_name,
          Tháng: row.month,
          Năm: row.year,
          "Thời gian": `${row.month}/${row.year}`,
          "Danh hiệu": row.title || "-",
          Hạng: row.rank ?? "-",
          Điểm: row.score ?? "-",
        });
  ...
  return (
    <div className="text-sm mx-auto p-6 bg-white border-[#ccc] rounded-[5px]">
      ...
      <div className="flex flex-wrap gap-4 mb-6 bg-gray-50 p-4 rounded border">
        ...
        <div>
          <label className="block text-xs text-gray-600 mb-1">Tên cuộc thi</label>
          <input
            type="text"
            placeholder="Nhập tên cuộc thi để lọc..."
